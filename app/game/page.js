...
export default function GamePage() {
  ...
  useEffect(() => {
    const fetchMarkets = async () => {
      try {
        setMarketsError("");
        const res = await fetch("/api/sports/matchlist");
        ...
        const isLiveMatch = (m) => isLiveFlag(m?.iplay);

        const filtered = list.filter((m) => {
          if (m?.status === "SUSPENDED") return false;
          if (isLiveMatch(m)) return true;
          const matchTime = parseMatchTime(m?.stime);
          if (!matchTime) return false;
          return matchTime.getTime() - now <= fourDaysMs;
        });

        const ordered = [
          ...filtered.filter((m) => isLiveMatch(m)),
          ...filtered.filter((m) => !isLiveMatch(m)),
        ];

        const mapped = ordered.slice(0, 5).map((m) => {
          const tags = [];
          if (m?.bm) tags.push("BM");
          if (m?.f) tags.push("F");
          if (m?.f1) tags.push("F1");
          if (m?.tv) tags.push("TV");

          const team1 = m?.section?.[0];
          const team2 = m?.section?.[1];

          const pickOdd = (team, type) =>
            team?.odds?.find((o) => o?.otype === type)?.odds ?? "-";

          return {
            id: String(m?.gmid || m?.mid || m?.ename),
            gmid: m?.gmid,
            date: m?.stime || "-",
            match: m?.ename || "Match",
            tags,
            isLive: isLiveMatch(m),
            back: [pickOdd(team1, "back"), "-", "-"],
            lay: [pickOdd(team2, "lay"), "-", "-"],
          };
        });

        setCricketMarkets(mapped);
      } catch (err) {
  ...
  return (
    <div
      className="relative flex h-auto min-h-screen w-full flex-col bg-[#f4f1ea] text-white antialiased overflow-x-hidden"
      style={{ fontFamily: "Arial, sans-serif" }}
    >
      {showWelcome && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="relative w-full max-w-sm rounded-3xl bg-white p-1 shadow-2xl">
            <button
              type="button"
              onClick={() => setShowWelcome(false)}
              className="absolute right-3 top-3 rounded-full bg-black/80 p-1 text-white hover:bg-black"
      ...
      <header className="relative flex items-center justify-between border-b border-black/10 px-6 py-4 md:px-20 lg:px-40 sticky top-0 bg-gradient-to-r from-[#071E46] to-[#0B3B8A] z-50">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            {/* Replaced Icon and Text with Logo Image */}
            <img
              src="/dice_rush_logo.png"
              alt="Dice Rush Logo"
              className="h-14 w-auto object-contain drop-shadow-[0_0_10px_rgba(57,255,20,0.6)]"
            />
          </div>
        </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-md">
            ...
            >
              {HERO_SLIDES.map((slide) => (
                <div
                  key={slide.id}
                  className="relative w-1/5 h-[160px] md:h-[240px] lg:h-[300px] flex-shrink-0 bg-black"
                >
                  <img
                    src={slide.image}
                    alt="Casino banner"
        ...
        <section className="mb-16">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              <Link
                href="/sports"
                className="text-xs font-bold text-slate-700 hover:text-slate-900 underline decoration-slate-300 underline-offset-4"
              >
                View All
              </Link>
          ...
          <div className="grid grid-cols-1 gap-3">
            {marketsLoading ? (
              <div className="rounded-xl border border-slate-200 bg-[#fbfaf7] px-4 py-6 text-center text-sm text-slate-500">
                Loading live markets...
              </div>
            ) : marketsError ? (
              <div className="rounded-xl border border-slate-200 bg-[#fbfaf7] px-4 py-6 text-center text-sm text-red-500">
                {marketsError}
              </div>
            ) : cricketMarkets.length === 0 ? (
              <div className="rounded-xl border border-slate-200 bg-[#fbfaf7] px-4 py-6 text-center text-sm text-slate-500">
                No live matches right now.
              </div>
            ) : (
              cricketMarkets.map((market) => (
                <Link
                  key={market.id}
                  href={`/sports/${market.gmid}`}
                  className="block"
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-4 rounded-xl border border-slate-200 bg-[#fbfaf7] px-4 py-3 shadow-sm transition hover:shadow-md hover:border-slate-300">
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-1 gap-4 text-[12px] leading-relaxed text-slate-600 md:grid-cols-3 md:gap-8">
              ...
              <div className="flex flex-col gap-1 border-l border-slate-200 pl-3">
                <span className="font-bold text-slate-800 underline decoration-slate-300 underline-offset-4">
                  01. Eligibility
                </span>
                <p>
                  Users must be 18+ to participate. Account verification is
                  mandatory for withdrawals. We reserve the right to restrict
              ...
              <div className="flex flex-col gap-1 border-l border-slate-200 pl-3">
                ...
                <p>
                  Instant joins require a minimum wallet balance. All stakes are
                  final once a round begins. Please review table limits before
                  joining.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-1 border-l border-slate-200 pl-3">
                <span className="font-bold text-slate-800 underline decoration-slate-300 underline-offset-4">
                  03. Responsible Gaming
                </span>
                <p>
                  Set your own limits. Our system monitors betting patterns to
                  ensure a safe environment. Visit our "Help" section for
