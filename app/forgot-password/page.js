...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/user-api/forgot-password', {
        method: 'POST',
      ...
      <div className="hidden lg:block lg:w-3/5 relative overflow-hidden group">
        ...
        <div className="absolute top-10 left-10 flex items-center gap-6">
          <div className="flex items-center gap-2 px-4 py-2 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full">
            <span className="text-[11px] font-bold tracking-widest uppercase">Reset Access</span>
          </div>
        </div>
        ...
        <div className="absolute bottom-20 left-16 max-w-md" style={{ padding: "8px" }}>
          <h1 className="text-7xl font-black tracking-tighter italic leading-none text-white drop-shadow-2xl">
            RESET<br /><span className="inline-block pb-1 text-transparent bg-clip-text bg-gradient-to-br from-yellow-200 via-yellow-500 to-yellow-800" style={{padding: "0px 10px"}}>DICE RUSH</span>
          </h1>
          <p className="mt-6 text-gray-400 font-medium leading-relaxed">
            Secure your account with a new password and roll back into the action.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-2/5 flex flex-col items-center justify-center relative p-6 md:p-12">
        ...
        >
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-bold text-gray-500 tracking-widest ml-1">New Password</label>
              <div className="relative">
          ...
          <div className="mt-4 pt-2 border-t border-white/5 text-center">
            <Link href="/login">
              <span className="text-xs text-gray-500 font-medium italic">
                Remembered? <span className="text-yellow-500 hover:text-yellow-300 font-bold transition not-italic">Back to login</span>
              </span>
            </Link>
          </div>
        </motion.div>
